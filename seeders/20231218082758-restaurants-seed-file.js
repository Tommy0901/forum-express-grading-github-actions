...
const { responseUrlHandler } = require('../helpers/download-helpers')
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const url = `https://loremflickr.com/320/240/food?/random=${Math.random() * 100}`
    const responseUrlArr = await Promise.all(Array.from({ length: 50 }).map(() => (responseUrlHandler(url))))
